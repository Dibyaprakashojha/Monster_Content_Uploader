...
getSessioons(){
    ...
    return this.http.get(baseUrl, httpOptions).pipe(map((response:any) => {
      this.jSession = response.session_resource.session.id;
...
startUpload(files:any, data :any, isRevision:any): Observable<any> {
  return new Observable(observer => {
    ...
    if (eventData.folderId) {
        if (!isRevision) {
            this.uploadFilesViaHTTP(files, eventData)
                .subscribe({
    ...
    console.log('dede',eventData)
              ...
              this.assetCheckout([data.assetData.assetId])//this.checkOutAsset([data.asset.asset_id])
                .subscribe({
                    ...
                    this.uploadVersionViaHTTP(files)
                        .subscribe({
                            next:(jobId: any) => {
                            const checkInData = {
                            ...
                            // eventData.folderId
                            this.assetCheckIn(checkInData, jobId )
                                .subscribe(
                                    {
...
uploadFilesViaHTTP(filesToUpload:any, eventData:any, isRevision?:any): Observable<any> {
  return new Observable(observer => {
      this.customImport(filesToUpload[0].name)
          .subscribe({
            ...
            console.log(createOTMMJobResponse)
              ...
              this.assetsRendition(filesToUpload,jobId).subscribe({
                next:()=>{
                  this.importOTMMJob(filesToUpload, eventData.otmmFields, eventData.otmmFieldValues,
                      eventData.folderId, eventData.templateid, eventData.metadataModel, eventData.securityPolicyID, jobId, isRevision)
                      .subscribe((importOTMMJobResponse:any) => {
...
let baseUrl='https://micromm.acheron-tech.com:11090'
  ...
  if (isRendition === true) {
      ...
      return baseUrl + config.config?.version+ '/' + otmmapiurl +
...
formAssetRepresentation(metadata:any, metaDataModel:any, securityPolicyID:any) {
  ...
  if (metadata && metaDataModel && metaDataModel != null && metaDataModel !== '') {
      ...
      if (securityPolicyID && securityPolicyID != null && securityPolicyID.length > 0) {
...
assetCheckout(assetIds: Array<string>) {
    return new Observable(observer => {
        if (!assetIds || !assetIds.length || assetIds.length === 0) {
            this.notificationService.error('No asset id(s) are available to check out.');
            observer.next(false);
            observer.complete();
        }
...
doPutRequest(url:any, parameters:any, headerObject:any, isOnlyResponse:any) {
    return new Observable(observer => {
        if (headerObject) {
...
uploadVersionViaHTTP(files:any): Observable<any> {
    return new Observable(observer => {
        this.assetImport(files[0].name)
            .subscribe((assetImportResponse:any) => {
                const jobId = assetImportResponse['job_handle'].job_id;
                this.assetsRendition(files, jobId)
                    .subscribe(assetsRenditionResponse => {
                        observer.next(jobId);
...
checkOutAsset(filesToCheckOut:any): Observable<any> {
    return new Observable(observer => {
        this.assetCheckout(filesToCheckOut)
            .subscribe(assetCheckoutResponse => {
                this.lockAssets(filesToCheckOut)
                    .subscribe((lockAssetResponse:any) => {
...
checkInAsset(filesToCheckIn:any, JobId :any, folderId:any): Observable<any> {
    return new Observable(observer => {
        this.assetCheckIn(filesToCheckIn, JobId)
            .subscribe((response:any) => {
                // const uploadDetails = {
...
doPostRequest(url:any, parameters:any, headerObject:any, isOnlyResponse:any) {
    return new Observable(observer => {
