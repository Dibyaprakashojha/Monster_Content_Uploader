...
export class PaginatorDirective implements AfterViewInit {
        ...
        for (let i = 0; i < nodeArray.length; i++) {
          if (nodeArray[i].nodeName === 'BUTTON') {
            if (nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled) {
              ...
              // this.ren.setStyle(nodeArray[i], 'background-color', '#8cc63f');
              this.ren.setStyle(nodeArray[i], 'color', 'white');
            }
          }
        }
  ...
  private createButton(i: any, pageIndex: number): any {
    ...
    this.ren.setStyle(linkBtn, 'background-color', '#8cc63f');
    this.ren.setStyle(linkBtn, 'border-width', '0px');
    this.ren.setStyle(linkBtn, 'border-radius', '50%');
